...
> 신경망(Neural Network)은 신경회로 또는 신경의 망으로, 현대적 의미에서는 인공 뉴런이나 노드로 구성된 인공 신경망을 의미한다.
...
그 이유는 바로 입력층과 출력층은 사람이 쉽게 결정할 수 있습니다. 예를 들면 사람의 손글씨 사진을 학습하여 0에서 9사이 범위의 숫자를 **분류**하는 신경망이 있다고 해봅시다.

이 신경망의 입력은 픽셀(pixel)단위로 나누어진 사람의 손글씨 사진의 데이터일 것이고 출력은 0에서 9사이 숫자 중 신경망이 추측한 정답이 포함된 배열일 것입니다.

하지만 은닉층의 값들은 어떤 역할을 하는지 쉽게 결정할 수 없습니다. 오로지 신경망 스스로만 알고 있고(private) 외부에서는 알 수 없기 때문에 ***숨겨져있다***(hidden)라고 표현하는 것이죠.
